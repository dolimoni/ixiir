				...
				<ul>
				    <li class='header-link {{$li_active_metier}}'>    
						<a href="{{route('postsMetier',!empty(Auth::user()->metierSpecialite['id'])?Auth::user()->metierSpecialite['id']:0)}}">
							...
							{{ !empty(Auth::user()->metierSpecialite)?(empty(session('lang'))?0:session('lang')==0?Auth::user()->metierSpecialite->nom_en:(empty(session('lang'))?0:session('lang')==1?Auth::user()->metierSpecialite->nom_ar:Auth::user()->metierSpecialite->nom_fr)):config('lang.lbl_mon_domaine')[empty(session('lang'))?0:session('lang')] }}
						</a>
					</li>
					<li class='header-link {{$li_active_city}}'>
						<a href="{{route('postsCity',!empty(Auth::user()->city['id'])?Auth::user()->city['id']:0)}}">
							...
							{{ !empty(Auth::user()->city)?Auth::user()->city->nom_en:config('lang.lbl_ma_ville')[empty(session('lang'))?0:session('lang')] }}
						</a>
					</li>
					<li class='header-link {{$li_active_country}}'>
						<a href="{{route('postsCountry',!empty(Auth::user()->country->id)?Auth::user()->country->id:0)}}">
							...
							{{ !empty(Auth::user()->country)?Auth::user()->country->nom_en:config('lang.lbl_mon_pays')[empty(session('lang'))?0:session('lang')] }}
						</a>
					</li>
