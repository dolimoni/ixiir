<?php
	...
	*/
...

	{
		...
		$dt_result=bddfetch($req, $pdo); 
		$req = 'SELECT post_id,detail

				FROM posts where a_modifier=3000 order by post_id desc;';
   
		$dt_result=bddfetch($req, $pdo); 
		$req="";
		foreach($dt_result as $post){
		    
		    $detail=$post['detail'];
		    $req.= "UPDATE posts SET detail='".injsql($detail,$pdo)."' where post_id=".$post['post_id'].";";
